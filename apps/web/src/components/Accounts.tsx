...
const CARD_ACCENTS: Record<string, { text: string; border: string; shadow: string }> = {
  ...
  savings: {
    ...
    shadow: "0 0 24px rgba(16,185,129,0.15)",
  },
  credit: {
    text: "text-amber-400",
    border: "border-amber-400/40",
    shadow: "0 0 24px rgba(251,191,36,0.15)",
  },
  exchange: {
...
function CheckingCard() {
  ...
  return (
    ...
    >
      ...
      <div className="bg-[#0a0a0a] rounded-lg p-4 font-[family-name:var(--font-geist-mono)] text-sm text-emerald mb-4 overflow-x-auto">
        <pre className="whitespace-pre leading-relaxed">{account.cli}</pre>
      </div>
      <div className="flex flex-wrap gap-3">
        {account.metrics.map((m) => (
          ...
          >
            ...
            <p className="text-sm font-[family-name:var(--font-geist-mono)] text-foreground font-medium">
              {m.value}
            </p>
          </div>
        ))}
...
function SavingsCard() {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-muted mb-4">{account.tagline}</p>
      {apyMetric && (
        <p className="text-3xl font-[family-name:var(--font-geist-mono)] text-emerald mb-3">
          {apyMetric.value}
        </p>
      )}
      {deposited && (
        <p className="text-sm text-muted">
          Deposited: <span className="text-foreground">{deposited.value}</span>
        </p>
      )}
      {earned && (
        <p className="text-sm text-muted mt-1">
          Earned: <span className="text-emerald">{earned.value}</span>
        </p>
      )}
    </motion.div>
  );
...
function SmallCard({
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-muted mb-4">{account.tagline}</p>
      {highlightMetric && (
        <p className={`text-3xl font-[family-name:var(--font-geist-mono)] ${accent.text}`}>
